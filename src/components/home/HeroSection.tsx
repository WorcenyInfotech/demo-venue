...
import { PLACEHOLDER_IMAGES, SITE_CONFIG } from "@/utils/constants";
...
const HERO_SLIDES = [
  {
    ...
    subtitle: `Experience the grandeur of your perfect wedding at ${VENUE_IDENTITY.name}, where every moment is crafted into a timeless memory.`,
  },
  {
    frameImage: PLACEHOLDER_IMAGES.venue2,
    title: "Your Royal",
    titleHighlight: "Celebration",
    subtitle: `${VENUE_IDENTITY.landArea} of pristine lawns, elegant halls, and a devoted team to orchestrate your dream celebration from first visit to farewell.`,
  },
  {
    frameImage: PLACEHOLDER_IMAGES.about,
    title: "Luxury Beyond",
    titleHighlight: "Imagination",
    subtitle: `${VENUE_CONTACT.state}'s most prestigious wedding destination — where timeless tradition meets refined modern hospitality.`,
  },
];
      ...
      <div className="relative z-10 mx-auto flex min-h-[100dvh] max-w-[1400px] flex-col justify-center gap-10 px-4 pb-28 pt-28 sm:gap-12 sm:px-6 lg:flex-row lg:items-center lg:gap-12 lg:px-8 lg:pb-20 lg:pt-32 xl:gap-16">
        ...
        <div className="flex w-full flex-1 flex-col justify-center lg:max-w-xl lg:flex-none xl:max-w-2xl">
          ...
          >
            ...
            <span>{`${VENUE_CONTACT.city}'s #1 Luxury Wedding Venue`}</span>
            <Star size={13} className="fill-rose-gold text-rose-gold" />
          </motion.div>
          ...
          >
            ...
            <span>{VENUE_CONTACT.street} · {VENUE_CONTACT.state}</span>
          </motion.div>
          ...
          >
            ...
            <div className="relative w-full overflow-hidden rounded-[2rem] shadow-[0_32px_90px_rgba(51,51,51,0.16)] ring-1 ring-rose-gold/10">
              ...
              <div className="relative aspect-[3/4] w-full lg:aspect-auto lg:h-[min(78dvh,760px)] lg:min-h-[520px] lg:max-h-[min(88dvh,820px)]">
                <AnimatePresence mode="wait">
                  ...
                  >
                    <Image
                      ...
                      alt={`${slide.titleHighlight} at ${VENUE_IDENTITY.name}`}
                      fill
                ...
                >
                  <p className="font-display text-base font-semibold text-ink sm:text-lg">{VENUE_IDENTITY.name}</p>
                  <p className="mt-1 text-sm text-ink/60">{VENUE_IDENTITY.tagline}</p>
                </motion.div>
              </div>
            </div>
